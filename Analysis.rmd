...
### Pre Process
```{r preProcess}
preProcessObj <- preProcess(train_data, method = c("center", "scale", "pca"))
```
...
**Prediction**
```{r}
cv_predict <- predict(fitModel_lda, cv_data)
...
**Prediction**
```{r}
cv_predict <- predict(fitModel_svm, cv_data)
...
**Prediction**
```{r}
cv_predict <- predict(fitModel_rf, cv_data)

cfmtrx_rf <- confusionMatrix(cv_y$classe, cv_predict)
cfmtrx_rf$overall
```